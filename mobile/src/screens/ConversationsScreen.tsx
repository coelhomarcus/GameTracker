...
export default function ConversationsScreen() {
  ...
  function renderItem({ item }: { item: ConversationSummary }) {
    ...
    return (
      <Pressable
        style={styles.row}
        onPress={() => navigation.navigate('ChatRoom', { conversationId: item.id, otherUsername: username })}
      >
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={!query.isFetching ? <Text style={styles.empty}>Nenhuma conversa ainda</Text> : null}
      />
    </View>
  );
}
